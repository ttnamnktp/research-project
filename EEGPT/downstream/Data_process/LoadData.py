...
class LoadBCIC_E(LoadData):
    ...
    def get_epochs(self, tmin=-4.5, tmax=5.0, bandpass = False,resample = None,baseline=None):
        ...
        ### ------------------------ Test load ------------------------
        # mat_data      = sio.loadmat(os.path.join(self.eeg_file_path,self.label_name), squeeze_me=True, struct_as_record=False)
        # data     = mat_data['data']
        # y_all = []
        # subjects = data if isinstance(data, np.ndarray) else [data]
        # for subject in subjects:
        #     for i, trial_start in enumerate(subject.trial):
        #         y_all.append(subject.y[i])
        # y_all = np.array(y_all)
        # if y_all.max() > 4:
        #     label_map = {769: 0, 770: 1, 771: 2, 772: 3}
        #     y_all = np.array([label_map[i] for i in y_all])
        # else:
        #     y_all = y_all - 1
        # self.y_labels = y_all
        ### ------------------------ Test load ------------------------
