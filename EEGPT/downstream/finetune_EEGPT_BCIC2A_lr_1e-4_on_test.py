...
class LitEEGPTCausal(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optimizer = torch.optim.AdamW([
            {"params": self.target_encoder.parameters(), "lr": 1e-4},
        ...
        lr_scheduler = torch.optim.lr_scheduler.OneCycleLR(optimizer, max_lr=[1e-4, 1e-3, 1e-3, 1e-3], steps_per_epoch=steps_per_epoch, epochs=max_epochs, pct_start=0.1)
...
def main():
    ...
    for i in range(1,10):
        ...
        checkpoint_callback = pl.callbacks.ModelCheckpoint(
            dirpath=f"./EEGPT/ds_checkpoints/finetune_EEGPT_BCIC2A_lr_1e-4_on_test/subject{i}",
        ...
        trainer = pl.Trainer(accelerator='cuda',
                            ...
                            logger=[pl_loggers.TensorBoardLogger('./logs/', name="finetune_EEGPT_BCIC2A_lr_1e-4_on_test_tb", version=f"subject{i}"), 
                                    pl_loggers.CSVLogger('./logs/', name="finetune_EEGPT_BCIC2A_lr_1e-4_on_test_csv", version=f"subject{i}")])
